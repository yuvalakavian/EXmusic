...
class MainFrame(wx.Frame):
    ...
    ##The constructor of the class, who takes the father, the id, title, location, size, style and name.
    def __init__(self,parent,title, pos, size, style, name, url=None, popup=True):

        wx.Frame.__init__(self, parent=parent, id=wx.ID_ANY,
                title=title,pos=pos,size=size,style=style,name=name)
        size=(1000,600)
        ...
        if TEST_EMBEDDING_IN_PANEL:
            ...
            self.mainPanel = wx.Panel(self.panel,size=(700,635),pos=(600,400))
...
class ClientHandler:
    ...
    def OnAddressChange(self, browser, frame, url):
        ...
        print("    url = %s" % url)
    #Action sub division of CEF, which receives the browser and the page title as parameters. The function checks if the video title represents completion and in accordance with the changing playNext changes the main frame. This is a function that connects the events of javascript video player controls and a Python list Hliikim.

    def OnTitleChange(self, browser, title):
        print("[wxpython.py] DisplayHandler::OnTitleChange()")
        print("    title = %s" % title)
        if title=="ended":
            MainFrame.playNext="Yes"
    ...
    def OnLoadError(self, browser, frame, errorCode, errorTextList, failedUrl):
        ...
        if errorCode == cefpython.ERR_ABORTED:
            ...
            return;
        customErrorMessage = "Videos Will Appear Here"
